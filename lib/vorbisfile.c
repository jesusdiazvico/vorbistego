...
static int _fetch_and_process_packet(OggVorbis_File *vf,
  ...
  int hide_method;      /* Default hiding method */
  int sync_method;      /* Default synchronization method */
...
/*   int scdds;            /\* Default data size for crypto-packets *\/ */
  ...
  while(1){
    if(vf->ready_state==STREAMSET){
      ...
      /* Dirty and far away from elegant, but quick and easy */      
      da = vf->vb.da;
      sfile = vf->vb.sfile;
      hide_method = vf->vb.hide_method;
      sync_method = vf->vb.sync_method;
      sigma = vf->vb.sigma;
      skey = vf->vb.skey;
      sca = vf->vb.sca;
      scmda = vf->vb.scmda;
      ...
      vf->vb.da = da;
      vf->vb.sfile = sfile;
      vf->vb.hide_method = hide_method;
      vf->vb.sync_method = sync_method;
      vf->vb.sigma = sigma;
      vf->vb.skey = skey;
      vf->vb.sca = sca;
      vf->vb.scmda = scmda;
